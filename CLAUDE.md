...
- **Weather**: Google Weather API (`weather.googleapis.com/v1/forecast/days:lookup`) — 10-day forecast, cached in IndexedDB (3h TTL)
...
- **Daily weather forecasts**: Weather badges in itinerary list view day headers showing high/low temp, icon, and precipitation. Uses Google Weather API (10-day forecast, 10k free calls/month). Geocodes first trip city via `geocodeCity()`. Cached in `weatherCache` IndexedDB table (Dexie v5) with 3-hour TTL, keyed by rounded coords + date. Graceful degradation: no badge for dates >10 days away, API errors, or missing key.
- **Daily journal**: Users can write journal entries for each day of their trip. A journal button (BookOpen icon) appears in each day's header in both the list view and calendar view. When clicked, it opens a dialog with a textarea for writing/viewing the day's journal entry. The button shows in primary color when there's an entry, gray when empty. Journal entries are stored in the `reminders` IndexedDB table (Dexie DB version 3), keyed by `[tripId+dayKey]`. Uses `useReminders` hook for live queries.
...
- **Import places**: Users can import a list of places from Google Maps direction URLs or plain text (place names / coordinates). Import dialog accessible via "Import" button in trip header. Supports two input modes (URL parsing, text/coordinate parsing) and two import modes (as activity events with configurable date/time/duration, or as saved places stored in IndexedDB). Places are geocoded via Google Geocoding API with trip city biasing. Saved places stored in `savedPlaces` IndexedDB table (Dexie DB version 6). Detects Google Maps shared list URLs (`/maps/placelists/`), shortened URLs (`maps.app.goo.gl`), and other non-direction URLs — shows instructional UI with "Open in Google Maps" + "Paste Place Names" buttons since list content can't be extracted client-side due to CORS. Text parser handles Google Maps list paste format: strips " · Category" suffixes, list prefixes (`1.`, `-`, `•`), and skips metadata lines (ratings, phone numbers, hours, URLs).
